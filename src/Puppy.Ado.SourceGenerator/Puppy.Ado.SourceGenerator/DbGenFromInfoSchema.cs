...
public sealed class DbGenFromInfoSchema : IIncrementalGenerator
{
    ...
    {
        var options = context.AnalyzerConfigOptionsProvider.Select((p, _) => new GeneratorOptions(p));
...
public static partial class {v.ClrName}_ViewClient
{{
    public static async Task<List<{dtoName}>> QueryAsync(DbConnection connection, CancellationToken ct = default)
    {{
...
public static partial class {p.ClrName}_ProcClient
{{
    public static async Task<List<{(hasResult is null ? "object" : outputDtoName)}>> ExecuteAsync(
...
{readBlock}
    ...
    static partial void OnBeforeExecute(DbCommand cmd);
